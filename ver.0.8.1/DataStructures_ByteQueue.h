#pragma once
namespace DataStructures{
	class ByteQueue{
		public:
		void ByteQueue(void);
		void Clear(char const*,uint);
		void DecrementReadOffset(uint);
		void GetBytesWritten(void);
		void IncrementReadOffset(uint);
		void PeekContiguousBytes(uint *);
		void Print(void);
		void ReadBytes(char *,uint,bool);
		void WriteBytes(char const*,uint,char const*,uint);
		void ~ByteQueue();
		void Add(HuffmanEncodingTreeNode * const&);
		void Clear(void);
		void Del(void);
		void Insert(HuffmanEncodingTreeNode * const&);
		void Pop(uint);
		void Push(ulong long const&,RakNet::InternalPacket * const&,char const*,uint);
		void PushSeries(ulong long const&,RakNet::InternalPacket * const&,char const*,uint);
		void Swap(uint,uint);
		void Insert(DataStructures::Heap<ulong long,RakNet::InternalPacket *,false>::HeapNode const&,char const*,uint);
		void ~List();
		void Insert(DataStructures::Map<RakNet::SystemAddress,DataStructures::ByteQueue *,&DataStructures::defaultMapKeyComparison<RakNet::SystemAddress>>::MapNode const&,uint,char const*,uint);
		void Insert(DataStructures::Map<int,RakNet::HuffmanEncodingTree *,&DataStructures::defaultMapKeyComparison<int>>::MapNode const&,char const*,uint);
		void Insert(DataStructures::Map<int,RakNet::HuffmanEncodingTree *,&DataStructures::defaultMapKeyComparison<int>>::MapNode const&,uint,char const*,uint);
		void Insert(DataStructures::RangeNode<RakNet::uint24_t> const&,char const*,uint);
		void Insert(DataStructures::RangeNode<RakNet::uint24_t> const&,uint,char const*,uint);
		void RemoveAtIndex(uint);
		void Insert(RakNet::InternalPacket * const&,char const*,uint);
		void Preallocate(uint,char const*,uint);
		void ~List();
		void GetIndexOf(RakNet::PluginInterface2 * const&);
		void Insert(RakNet::PluginInterface2 * const&,char const*,uint);
		void Insert(RakNet::RakNetGUID const&,char const*,uint);
		void Insert(RakNet::RakNetSmartPtr<RakNet::RakNetSocket> const&,char const*,uint);
		void operator=(DataStructures::List<RakNet::RakNetSmartPtr<RakNet::RakNetSocket>> const&);
		void ~List();
		void Insert(RakNet::RakString::SharedString * const&,char const*,uint);
		void ~List();
		void Insert(RakNet::ReliabilityLayer::UnreliableWithAckReceiptNode const&,char const*,uint);
		void Insert(RakNet::SplitPacketChannel * const&,uint,char const*,uint);
		void Insert(RakNet::SystemAddress const&,char const*,uint);
		void Insert(StrAndBool const&,char const*,uint);
		void Insert(StrAndBool const&,uint,char const*,uint);
		void Insert(bool const&,char const*,uint);
		void Insert(uint const&,char const*,uint);
		void Preallocate(uint,char const*,uint);
		void Clear(void);
		void operator=(DataStructures::Map<RakNet::SystemAddress,DataStructures::ByteQueue *,&DataStructures::defaultMapKeyComparison<RakNet::SystemAddress>>::MapNode const&);
		void NodeComparisonFunc(RakNet::SystemAddress const&,DataStructures::Map<RakNet::SystemAddress,DataStructures::ByteQueue *,&DataStructures::defaultMapKeyComparison<RakNet::SystemAddress>>::MapNode const&);
		void Clear(void);
		void Get(int const&);
		void Has(int const&);
		void NodeComparisonFunc(int const&,DataStructures::Map<int,RakNet::HuffmanEncodingTree *,&DataStructures::defaultMapKeyComparison<int>>::MapNode const&);
		void Set(int const&,RakNet::HuffmanEncodingTree * const&);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::InternalPacket>::Page *,DataStructures::MemoryPool<RakNet::InternalPacket>::Page *,char const*,uint);
		void Release(RakNet::InternalPacket*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::InternalPacketRefCountedData>::Page *,DataStructures::MemoryPool<RakNet::InternalPacketRefCountedData>::Page *,char const*,uint);
		void Release(RakNet::InternalPacketRefCountedData*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::Packet>::Page *,DataStructures::MemoryPool<RakNet::Packet>::Page *,char const*,uint);
		void Release(RakNet::Packet*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::RakPeer::BufferedCommandStruct>::Page *,DataStructures::MemoryPool<RakNet::RakPeer::BufferedCommandStruct>::Page *,char const*,uint);
		void Release(RakNet::RakPeer::BufferedCommandStruct*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::RakPeer::RecvFromStruct>::Page *,DataStructures::MemoryPool<RakNet::RakPeer::RecvFromStruct>::Page *,char const*,uint);
		void Release(RakNet::RakPeer::RecvFromStruct*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::RakPeer::SocketQueryOutput>::Page *,DataStructures::MemoryPool<RakNet::RakPeer::SocketQueryOutput>::Page *,char const*,uint);
		void Release(RakNet::RakPeer::SocketQueryOutput*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::ReliabilityLayer::MessageNumberNode>::Page *,DataStructures::MemoryPool<RakNet::ReliabilityLayer::MessageNumberNode>::Page *,char const*,uint);
		void Release(RakNet::ReliabilityLayer::MessageNumberNode*,char const*,uint);
		void Clear(char const*,uint);
		void Release(RakNet::RemoteClient **,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::RemoteSystemIndex>::Page *,DataStructures::MemoryPool<RakNet::RemoteSystemIndex>::Page *,char const*,uint);
		void Release(RakNet::RemoteSystemIndex*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void InitPage(DataStructures::MemoryPool<RakNet::SystemAddress>::Page *,DataStructures::MemoryPool<RakNet::SystemAddress>::Page *,char const*,uint);
		void Release(RakNet::SystemAddress*,char const*,uint);
		void SystemAddress const&,bool *,int (*);
		void ~OrderedList();
		void uint24_t const&,bool *,int (*);
		void uint24_t> const&,bool,char const*,uint,int (*);
		void StrAndBoolComp(char * const&,StrAndBool const&);
		void GetIndexFromKey(int const&,bool *,int (*);
		void ~OrderedList();
		void GetIndexFromKey(ushort const&,bool *,int (*);
		void Push(HuffmanEncodingTreeNode * const&,char const*,uint);
		void Push(RakNet::BPSTracker::TimeAndValue2 const&,char const*,uint);
		void Pop(void);
		void Push(RakNet::InternalPacket * const&,char const*,uint);
		void RemoveAtIndex(uint);
		void Size(void);
		void Pop(void);
		void Push(RakNet::Packet * const&,char const*,uint);
		void PushAtHead(RakNet::Packet * const&,uint,char const*,uint);
		void Size(void);
		void operator[](uint);
		void ~Queue();
		void Pop(void);
		void Push(RakNet::RakPeer::BufferedCommandStruct * const&,char const*,uint);
		void Pop(void);
		void Push(RakNet::RakPeer::RecvFromStruct * const&,char const*,uint);
		void Push(RakNet::RakPeer::RequestedConnectionStruct * const&,char const*,uint);
		void RemoveAtIndex(uint);
		void Size(void);
		void operator[](uint);
		void Push(RakNet::RakPeer::SocketQueryOutput * const&,char const*,uint);
		void Pop(void);
		void Push(RakNet::ReliabilityLayer::DatagramHistoryNode const&,char const*,uint);
		void Size(void);
		void operator[](uint);
		void Push(RakNet::SystemAddress * const&,char const*,uint);
		void Pop(void);
		void Push(RakNet::SystemAddress const&,char const*,uint);
		void ~Queue();
		void Pop(void);
		void Push(bool const&,char const*,uint);
		void Size(void);
		void Clear(void);
		void Deserialize(RakNet::BitStream *);
		void Insert(RakNet::uint24_t);
		void Serialize(RakNet::BitStream *,uint,bool);
		void ~RangeList();
		void uint24_t>(RakNet::uint24_t const&,DataStructures::RangeNode<RakNet::uint24_t> const&);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void Deallocate(RakNet::RakPeer::BufferedCommandStruct*,char const*,uint);
		void Push(RakNet::RakPeer::BufferedCommandStruct*);
		void Clear(char const*,uint);
		void Deallocate(RakNet::RakPeer::RecvFromStruct*,char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void Deallocate(RakNet::RakPeer::SocketQueryOutput*,char const*,uint);
		void Pop(void);
		void Push(RakNet::RakPeer::SocketQueryOutput*);
		void Clear(char const*,uint);
		void Allocate(char const*,uint);
		void Clear(char const*,uint);
		void Deallocate(RakNet::SystemAddress*,char const*,uint);
		void PopInaccurate(void);
		void Push(RakNet::SystemAddress*);
		void ThreadsafeAllocatingQueue(void);
		void ~ThreadsafeAllocatingQueue();
	}
};
